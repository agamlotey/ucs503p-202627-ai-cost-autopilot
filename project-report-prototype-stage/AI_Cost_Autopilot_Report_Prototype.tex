...
The trimmer's token budget is set to the same 1000 tokens, so whenever the
autopilot asks for trimming, trimming happens. An earlier default of 8000 meant a
realistic 3,000-token coding request was planned for trimming but forwarded
untouched; the live dashboard exposed this, because it showed zero savings for
such requests.
...
\begin{itemize}
    ...
    \item \textbf{The token budget decides whether the trimmer runs.} The trimmer
          only acts above its budget, now 1000 tokens to match the autopilot. At
          the earlier 8000, the \code{notes\_api} request (3,277 tokens) was not
          trimmed at all through the gateway. With the aligned budget, sending it
          through the running gateway trims it from 3,348 to 2,237 tokens, the
          same saving the benchmark reports.
\end{itemize}
...
\begin{itemize}
    ...
    \item The autopilot's thresholds are fixed; tuning them from measured
          savings is not built yet.
    \item Metrics (DFD process 6.0) are recorded in memory for the live dashboard, but not yet persisted.
...
\begin{itemize}
    ...
    \item Autopilot threshold tuning from measured savings.
