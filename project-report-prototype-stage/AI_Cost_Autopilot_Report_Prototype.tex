...
\begin{titlepage}
    ...
    \end{tabular}\\[0.5cm]
    {\normalsize Computer Science and Engineering Department (CSED)}\\[1.6cm]
...
\begin{figure}[H]
    ...
    \caption{Request flow through the gateway.}
...
against all functions defined anywhere in it.
...
and only similarity matching is disabled.

\subsection{Module 3: Autopilot}

At this stage the autopilot is a baseline rule: always try the cache, and trim
when the request contains code. The trimmer itself leaves requests under the token
budget (8000 tokens by default) unchanged. The planned policy adds a safety check
that skips caching when a request contains secrets or personal data, and tunes the
trimming thresholds from measurements.
...
The test suite has 44 tests: 19 for the trimmer, 17 for the cache, 1 for the
autopilot and 7 for the gateway, most of which send real HTTP requests through the
whole pipeline using the mock provider. At the time of writing, 43 pass and 1 is skipped
...
same request, for the task ``fix \code{create\_note}''. ``Valid'' means every
output file still compiles as Python.
...
\textbf{Trimmer (focus and callees, 2 hops)}     & \textbf{2,232} & \textbf{31.9\%} & \textbf{valid} \\
\bottomrule
\end{tabular}
...
from another file, and collapsed the other 27 to their signatures.
...
We replayed a short coding session (the workload from the cache savings benchmark)
through the gateway logic and varied how many requests were exact repeats of
earlier ones (Table~\ref{tab:repeat}).
...
\begin{itemize}
    \item \textbf{The trimmer beats text stripping and stays correct.} Stripping
          ...
          trimmer saves 31.9\% while keeping everything the task depends on.
    ...
    \item \textbf{Correctness costs savings, and that is the right trade.} Before
          the cross-file fix, the trimmer saved 51.8\% on the same task, but only
    ...
    \item \textbf{The token budget decides whether the trimmer runs.} The trimmer
          ...
          explicitly. Choosing the budget is part of the next stage.
\end{itemize}
...
matches for code, and a baseline autopilot. It has 44 automated tests running in
...
\begin{enumerate}
    \item On a realistic request, the trimmer cuts 31.9\% of tokens and the output
...
\begin{itemize}
    ...
    \item The autopilot is a baseline rule; its safety check and tuning are not
          built yet.
    ...
    \item The trimmer's savings are measured on one fixture and one task so far.
\end{itemize}
...
\begin{itemize}
    ...
    \item The full autopilot policy, including secret and personal-data detection.
