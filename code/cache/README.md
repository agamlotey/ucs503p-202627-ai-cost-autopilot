...
- [x] Add a TTL + size-cap eviction (store no longer grows unbounded). *(TTL is sliding: a hit resets the timer, so it bounds memory but isn't a freshness guarantee.)*
